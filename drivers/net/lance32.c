 ...
 *
 *      Horribly hacked for SNI RM200 by Ralf Baechle.  This is currently
 *      classified as better than nothing software.  This doesn't mean that
 *      the driver is unreliable; it just means that it's horribly hacked
 *      code.  Also this driver can currently no longer be used for anything
 *      else but a SNI RM.  This driver will fail on RM configurations that
 *      map the PCnet to another address than expected.  I'll work on a
 *      better detection routine.  The cacheflush() calls are far too many;
 *      also they're placed braindead and too paranoid.
 */

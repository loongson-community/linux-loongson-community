...
int au1x00_pcmcia_socket_probe(struct device *dev, struct pcmcia_low_level *ops, int first, int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		if (i == 0) {
			...
			skt->phys_attr = (ioaddr_t)AU1X_SOCK0_PHYS_ATTR;
			skt->phys_mem = (ioaddr_t)AU1X_SOCK0_PHYS_MEM;
		}
			...
			skt->virt_io = (void *)
				...
				(u32)mips_io_port_base);
			skt->phys_attr = (ioaddr_t)AU1X_SOCK1_PHYS_ATTR;
			skt->phys_mem = (ioaddr_t)AU1X_SOCK1_PHYS_MEM;
		}
#endif

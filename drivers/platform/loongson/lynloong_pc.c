/*
 *  Driver for LynLoong laptop extras
 *
...
static void set_gpio_low(int gpio)
{
	GPIO_HI_BIT(gpio, GPIOL_OUT_EN);
	GPIO_LO_BIT(gpio, GPIOL_OUT_VAL);
}

static void set_gpio_high(int gpio)
{
	GPIO_HI_BIT(gpio, GPIOL_OUT_EN);
	GPIO_HI_BIT(gpio, GPIOL_OUT_VAL);
}
...
static void setup_mfgpt2(void)
{
	...
	outw(0, MFGPT2_CNT);
	/* enable counter, output high on great than or equal(compare mode), 32k */
...
static int lynloong_thermal_init(struct device *dev)
{
	...
	lynloong_thermal_cdev = thermal_cooling_device_register("LCD",
								dev,
								&video_cooling_ops);
...
static int lynloong_suspend(struct platform_device *pdev, pm_message_t state)
{
	int i;
...
static int lynloong_resume(struct platform_device *pdev)
{
	int i = 1;
	...
	/* resume clock frequencies and enable the relative clocks of some devices */
	resume_clk_freq();
	enable_clocks();
...
static int __init lynloong_init(void)
{
	...
	if (ret) {
		...
		printk(KERN_INFO
		       "init lynloong thermal cooling device(LCD backlight) failure\n");
		return ret;
	}
...
MODULE_DESCRIPTION("LynLoong Platform Specific Driver; Xiang Yu <xiangy@lemote.com>");
MODULE_LICENSE("GPL");

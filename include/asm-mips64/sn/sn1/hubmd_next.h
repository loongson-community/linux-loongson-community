...
#ifdef BRINGUP
/* XXX moved over from SN/SN0/hubmd.h -- each should be checked for IP35 */
...
/* XXX protection and migration are completely revised on IP35.  On
   IP27, the reference count and protection fields were accessed in the
   same word, but on IP35 they reside at different addresses.  The
   users of these macros will need to be rewritten.  Also, the MD page
   size is 16K on IP35 but 4K on IP27.  */
...
/* Like IP27, IP35 supports a mostly-flat address space with 8
   CPU-visible, evenly spaced, contiguous regions, or "software
   banks".  On IP35, software bank n begins at addresses n * 1GB, 
   0 <= n < 8.

   Physically (and very unlike IP27), each IP35 node board contains 8
 ...
 *  #ifdefs for IP27 and IP35 in memerror.c code.  See SN0/hubmd.h
 *  for corresponding IP27 definitions.
 */
